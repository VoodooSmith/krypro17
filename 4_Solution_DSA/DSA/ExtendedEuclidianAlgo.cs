...
public class ExtendedEuclidianAlgo
{
    public static BigInteger ExtendedEuclid(BigInteger n, BigInteger m)
    {
        ...
        BigInteger q = 0, temp = 0, future_n = 0;            // "stringq" is the quotient, temp is a buffer
        ...
        while (m != 0)
        {
            ...
            x = x * (q * (-1));               // Matrix transformation - multiplication with negative "stringq"
            x = x + a;                        // Matrix transformation - Addition "x" and "a" for new "x"

            temp_y = y;                       // y is getting stored for later use 
            y = y * (q * (-1));               // Matrix transformation - Multiplication with negative "stringq"
